...
function sortChapterValue(chapter: ChapterItem) {
  const value = parseFloat((chapterNumber(chapter) || "0").replace("-", "."));
...
export default async function MangaDetailPage({
  ...
  const comicType = comic.info?.["Jenis Komik"] || comic.info?.Type || "";
        ...
        <section className="mt-4 grid gap-6 rounded-xl border border-zinc-200 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-zinc-900/70 sm:p-5 lg:grid-cols-[300px_1fr]">
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              {firstNumber ? (
                <Link
                  href={buildChapterHref(slug, firstNumber)}
              ...
              {latestNumber ? (
                <Link
                  href={buildChapterHref(slug, latestNumber)}
